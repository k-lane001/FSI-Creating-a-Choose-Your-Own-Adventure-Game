...
Do you (follow it), or (continue) on your path?`)
    if(secondAnswer === 'follow it') {
        let thirdAnswer = window.prompt('You follow the cat to a colony of cats, nestled in a fort of warm blankets and subsisting off of inexplicably warm soup. They are content with you staying (stay), but you wonder if you should alert the world to this magical safe haven (spread the word).')
        if(thirdAnswer === 'stay') {
          ...
          if(thirdAnswer === 'ladder') {
                ...
                window.alert('After ascending the staircase, you discover ashiny blue stone, which you take home and cherish forever.');
            }
    }
} 
...
lead you away from the dragon altogether. Which path do you take?(past the dragon)(away from the dragon)`)
    if(secondAnswer === 'past the dragon') {
        ...
        let thirdAnswer = window.prompt('After walking a while longer, you come across a shiny blue flower.  It is so beautiful that you decide you must either (draw it) or (pick it).  Which do you do?')
            if(thirdAnswer === 'draw it') {
                window.alert('You draw the flower, capturing only a fraction of its beauty with your quill. You bring the drawing home, somewhat disappointed, but over time, discover joy in sharing it with your friends and family, recounting the story of your days as a socrcerer with the aid of the sketch.')
            } else if(thirdAnswer === 'pick it') {
                window.alert('You pic the flower and bring it home, and all marvel at its brilliance.  However, over time it fades and eventually crumbles to dust.')
